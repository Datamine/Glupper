...
async def create_post(user_id: UUID, content: str, media_urls: Optional[List[str]] = None) -> Dict:
    ...
    async with pool.acquire() as conn:
        ...
        """, post_id, user_id, content, media_urls, now, now)
...
async def create_comment(
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            """, comment_id, user_id, content, media_urls, post_id, now, now)
...
async def repost(user_id: UUID, original_post_id: UUID) -> Optional[Dict]:
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            """, repost_id, user_id, original_post["content"], original_post["media_urls"], 
                True, original_post_id, now, now)
